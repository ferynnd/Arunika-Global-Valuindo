...
export default function Edit({ adminData, roles }) {
    const { data, setData, put, processing, errors } = useForm({
        ...
        role: adminData.role || roles[0]?.name || 'admin',
        is_active: adminData.is_active ?? true,
    });

    const submit = (e) => {
        e.preventDefault();
        put(route('admin.users.update', adminData.id), {
            onError: () => {
                showErrorAlert(
                    'Gagal Memperbarui Pengguna!',
                    'Silakan periksa kembali kolom isian yang wajib diisi.'
                );
            },
        });
    };
    ...
    return (
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                        ...
                        <p className="text-xs sm:text-sm text-slate-500 mt-0.5">
                            Perbarui informasi data diri atau hak akses untuk <span className="font-semibold text-primary">@{adminData.username}</span>.
                        </p>
                    </div>
                    <Link
        ...
        >
            <Head title={`Edit Pengguna: ${adminData.name} - Admin Arunika`} />
                ...
                <div className="w-full max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                    <form onSubmit={submit} className="bg-white rounded-2xl border border-slate-200/80 p-6 sm:p-8 space-y-6 shadow-xs w-full">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label className="inline-flex items-center text-sm font-semibold text-slate-700 mb-1.5">
                                    ...
                                    <InfoTooltip text="Menentukan hak akses menu pengelolaan pengguna ini." />
                                </label>
                                <select
                                    value={data.role}
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-3 border-t border-slate-100">
                            <div>
                                <label className="inline-flex items-center text-sm font-semibold text-slate-700 mb-1.5">
                                    <span>Kata Sandi Baru <span className="text-xs font-normal text-slate-400">(Opsional)</span></span>
                                </label>
                                <input
                                    type="password"
                                    value={data.password}
