                    ...
                    <div className="bg-white rounded-3xl border border-[#EAE6DF] p-5 shadow-sm">
                        <form onSubmit={handleSearch} className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                            ...
                            <div className="flex gap-2">
                                <button
                                    type="submit"
                                    className="flex-1 rounded-xl bg-[#1B544D] text-white font-semibold text-xs sm:text-sm px-4 py-2 hover:bg-[#15433E] transition-colors"
                                >
                                    Cari
                                </button>
                                {(search || statusFilter) && (
                                    <Link
                                        ...
                                        className="rounded-xl bg-slate-100 text-slate-600 font-semibold text-xs sm:text-sm px-4 py-2 hover:bg-slate-200 transition-colors flex items-center justify-center"
                                    >
                                        Reset
                                    </Link>
                                )}
                    ...
                    <div className="bg-white rounded-3xl border border-[#EAE6DF] shadow-sm overflow-hidden">
                        <div className="overflow-x-auto">
                            <table className="w-full text-left border-collapse text-xs sm:text-sm">
                                <thead>
                                    <tr className="bg-[#FAF8F5] border-b border-[#EAE6DF] text-[#1B544D] font-bold uppercase tracking-wider text-[11px]">
                                                    ...
                                                    {service.features && service.features.length > 0 ? (
                                                        <div className="flex flex-wrap gap-1 max-w-xs">
                                                            {service.features.slice(0, 3).map((feat, idx) => (
                                                                <span key={idx} className="px-2 py-0.5 rounded-md bg-[#FAF8F5] text-[11px] text-[#1B544D] border border-[#EAE6DF]">
                                                                    ✓ {feat}
                                                                </span>
                                                            ))}
                                    ...
                                    {services.links.map((link, key) => (
                                        link.url ? (
                                            ...
                                            <span
                                                key={key}
                                                dangerouslySetInnerHTML={{ __html: link.label }}
                                                className="px-3 py-1.5 rounded-lg text-xs text-slate-300 border border-[#EAE6DF]"
                                            />
                                        )
                                    ))}
                                </div>
